...
- **Frontend** (`useLea`, `Lea2View`) envoie à chaque message :
  ...
  - `transaction_id` : optionnel, présent quand on ouvre Léa depuis la fiche transaction (`/lea2?transaction=57`)
...
- **GET /api/v1/transactions/:id** : renvoie la transaction avec `sellers` et `buyers`. Le schéma `RealEstateTransactionResponse` impose des listes (validator qui remplace `null` par `[]`).
- **GET /api/v1/transactions/:id/steps** : renvoie une vue « steps » avec `buyer` = premier nom des acheteurs, `seller` = premier nom des vendeurs (pour la carte récap).
- **Frontend** :
  - **Page détail** (`/dashboard/transactions/[id]`) : en-tête avec « Vendeurs : » / « Acheteurs : » depuis `transaction.sellers` / `transaction.buyers` ; onglet Contacts (liste API + repli sur `transaction.sellers`/`buyers`).
  - **Carte « Transaction Achat »** (dans l’onglet Étapes) : données fournies par l’API steps → `transaction.buyer` et `transaction.seller`.
