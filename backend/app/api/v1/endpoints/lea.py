...
LEA_SYSTEM_PROMPT = (
    ...
    "** CONFIRMATION DE LA PROPRIÉTÉ : ** Quand l'utilisateur désigne une propriété par une référence partielle (ex. « celle sur de Bordeaux », « la transaction rue X ») et que tu identifies le bien dans les Données plateforme, tu DOIS d'abord confirmer explicitement la propriété avant de poser la question suivante (vendeurs, acheteurs, etc.). Formule par exemple : « D'accord, nous travaillons sur la propriété au [adresse complète]. Qui sont les vendeurs pour ce dossier ? » — ne passe jamais directement à « Qui sont les vendeurs ? » sans avoir confirmé clairement le bien concerné dans ta réponse.\n\n"
