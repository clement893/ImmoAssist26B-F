...
class Settings(BaseSettings):
    ...
    # Database (str to support SQLite; PostgresDsn used only for postgres)
    DATABASE_URL: Union[str, PostgresDsn] = Field(
        default=os.getenv("DATABASE_URL", "postgresql+asyncpg://user:CHANGE_PASSWORD@localhost:5432/modele"),
    ...
    def assemble_db_connection(cls, v: str | PostgresDsn) -> str:
        if isinstance(v, str):
            ...
            if "sqlite" not in v.lower() and v.startswith("postgresql://") and "+" not in v:
    ...
    LEA_TTS_SPEED: float = Field(
        ...
        ge=0.25,
        le=2.0,
        description="Vitesse de parole Léa (1.0 = normal, 1.35 = lecture accélérée)",
    )
